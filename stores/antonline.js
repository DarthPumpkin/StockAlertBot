...
export default async function antonline(url, interval) {
	...
	try {
		...
		if (res && res.status === 200) {
			...
			let title = document_.querySelectorAll(".title");
			let inventory = document_.querySelectorAll(".uk-button.uk-button-primary.add_to_cart");
			let image = document_.querySelectorAll(".uk-slideshow");

			if (title.length > 0) title = title[0].innerHTML.slice(0, 150);
			if (image.length > 0) {
				image = parser.parseFromString(image[0].innerHTML, "text/html");
				image = image.querySelectorAll("img");
				image = image[0].getAttribute("src");
			} else {
				image = document_.querySelectorAll(".main_img");
