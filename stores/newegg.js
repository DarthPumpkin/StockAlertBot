...
export default async function newegg(url, interval) {
	...
	try {
		...
		if (res && res.status == 200) {
			...
			let document_, title, inventory, image;

			// Check combo product
			if (url.includes("ComboDealDetails")) {
				document_ = parser.parseFromString(html, "text/html");
				title = document_.querySelectorAll("title")[0].textContent;
				inventory = document_.querySelectorAll(".atnPrimary");
				image = "https:" + document_.querySelector("#mainSlide_0").getAttribute("src");
			} else {
				// Check normal product
				document_ = parser.parseFromString(html, "text/html");
				title = document_.querySelectorAll(".product-title")[0].innerHTML.trim().slice(0, 150);
				inventory = document_.querySelectorAll(".btn.btn-primary.btn-wide");
				image = document_.querySelectorAll(".image_url");
