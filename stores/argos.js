...
export default async function argos(url, interval) {
	try {
		...
		if (res && res.status == 200) {
			...
			let document_ = parser.parseFromString(res.data, "text/html");
			let title = document_.querySelectorAll(".Namestyles__Main-sc-269llv-1");
			let inventory = document_.querySelectorAll(".xs-8--none");
			let image = document_.querySelectorAll(".MediaGallerystyles__ImageWrapper-sc-1jwueuh-2");

			if (title.length > 0) title = title[0].firstChild.textContent.trim().slice(0, 150);
			else {
				title = document_.querySelector("#h1title").textContent;
			...
			if (image.length > 0) {
				image = image[0].querySelectorAll("img");
