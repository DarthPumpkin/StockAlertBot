	...
	if (url.includes("tescopreorders")) tescoPS5Preorder(url, interval);
		...
		try {
			...
			if (res && res.status == 200) {
				...
				let title = document_
					.querySelectorAll(".product-details-tile__title")[0]
					.innerHTML.trim()
					.slice(0, 150);
				let inventory = document_.querySelectorAll(".button.small.add-control.button-secondary")[0]
				)[0].innerHTML;
				let image = document_
					.querySelectorAll(".product-image.product-image-visible")[0]
					.getAttribute("src");
