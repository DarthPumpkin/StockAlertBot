...
export default async function sendAlertToSMSViaEmail(product_url, title, image, store) {
	if (SMS_METHOD == "Email") {
		if (!fs.existsSync(".env")) {
			console.error(
				moment().format("LTS") + ": Error sending SMS alert, rename example.env file to .env"
			);
		} else if (sms_email.number.length === 0 && (!sms_email.from || !sms_email.pass)) {
			console.error(
				moment().format("LTS") + ": For SMS alerts to work, both email and SMS must be configured"
			);
		} else {
